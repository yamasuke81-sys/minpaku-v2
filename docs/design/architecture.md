# 当初アーキテクチャ設計書（2026-04 初版）

> 旧 CLAUDE.md（2026-07-02 ダイエット）から無損失で移設した当初設計。
> 注意: BEDS24 連携は構想のまま**未実装**で、実際の予約同期は iCal（Airbnb + Booking.com、functions/scheduled/syncIcal.js）ベース。
> デプロイ運用も現在は v2-5-relay がメイン（CLAUDE.md の 🚩 セクションが正）。歴史的経緯・全体像の参照用。
