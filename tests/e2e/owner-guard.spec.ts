...
// オーナーの ID トークン取得（テスト用: Service Account 経由カスタムトークン）
// ローカル実行時は GOOGLE_APPLICATION_CREDENTIALS 環境変数を設定すること
...
// SA なしのローカル環境では TC-OG1/OG3 をスキップ
...
test("TC-OG1: オーナー uid に role=staff を設定すると 400 が返る", async () => {
  test.skip(!hasSa, "GOOGLE_APPLICATION_CREDENTIALS が未設定のため SA なし環境ではスキップ");
  const db = getDb();
