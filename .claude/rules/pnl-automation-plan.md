...
## 調査結果(2026-07-14 My docomo API 探索 — 方針提案中)
- **Booking.com 再ログイン完了・実証済(NEXT#2消化)**: yadozei-relogin フロー(pm2 stop→--login→ブラウザクローズ検知→pm2 start)で再ログイン。**sessionCheck が Booking.com=ok に復帰**(8/1期限タスク完了)。extranet 画面がすぐログアウトして見えるのはUIのアイドルタイムアウトで、自動取得用cookieは有効。
...
## DONE(2026-07-14 ドコモ光 月額判明・6月計上 — やますけがMy docomo画面提示)
やますけが My docomo「詳細内訳」画面(電話番号=光回線 F5392930898、6月ご利用分)を提示 → **月額完全判明**:
- **ドコモ光 2026-06分 = ¥6,636(税込・確定)**: 基本使用料5,200(戸建・タイプA/西2)+回線工事料分割916(総額2万円/全24回・2026-05請求開始・6月時点残債20,152)+消費税520。エディオンネット利用=0(基本料内包)。支払=マリオットAMEX。**定常額(工事完済=2028年頃以降)=¥5,720**。
- **6月 the Terrace Wi-Fi・通信費 ¥1,200(暫定手入力)→¥6,636 に是正計上**(creditCardIndex sourceId=docomo-hikari:2026-06、内訳note保存)。**6月利益 ¥341,466→¥336,030(79.7%)**、報告書再生成済。
- **請求内訳の実URL判明=payment2.smt.docomo.ne.jp/spguide/ebilling/gkfap001.srv**(前回mydocomo.docomo.ne.jpを掘っていたが実体は別ドメインのeビリング)。**自動化は原理的に可能だが、CDP接続がユーザーのブラウザ操作と競合(WS接続後にセッション確立でtimeout)して今回は自動取得できず**。毎朝ルーチン(ユーザー非操作時)なら取れる見込み → SPA解析して mf-komachi-utilities に「ドコモ光」追加は次NEXT(優先度中、金額は定額なので手動確定でも実害小)。
- **残: 5月分(開通月5/19・課金5月開始)の光額が未取得**(日割りor初月無料の可能性)。My docomoの月タブ「5月」で確認 or 自動化完成後に取得 → 判明したら 5月 the Terrace 通信費に計上。
- **工事料分割916の扱い**: 当面は実請求額(通信費)に含めて計上(実支払一致で検算容易)。厳密には工事料の資産計上判断あり=税理士確認事項(少額のため実害小)。

## 調査完了(2026-07-14 ドコモ光 月額・支払い元を4ソースで調査 → 支払い元判明・月額は要人力確認)
やますけ「わからない、両方調べて」→ **4ソース全当たり**の結果:
- **✅支払い元=判明**: マリオットAMEX(恭介個人)。MF全口座スキャンで「ドコモご利用料金」引落は **AMEXのみ**(2026-03-31 ¥13,407 / 04-30 ¥7,213)。5月開通後(5/19)の引落はMF未反映(AMEX明細の取得ラグ、8月頃に5-6月分が乗る見込み)。
- **契約内容(判明分)**: ドコモ光コラボプラン(PPPoE戸建て)/契約者=西山恭介/設置=広長浜(the Terrace)/**5/19開通・5月課金開始**/支払先=NTTドコモ。エディオンネットサービス基本料は月額に内包。
- **❌月額(光単独)=確定できず**: 4ソース全滅 — ①契約確認書・契約内容書=金額マスク(「**円」表記、後日ドコモ書面参照)②開通案内=金額記載なし ③Gmail(yamasuke81/81hassac)=ドコモ請求メール0件(eビリング未設定?)④My docomo=7月携帯請求¥4,122に光含まれず(includesDenkiHikariFlag=false=別建て請求)・料金内訳ページはSPA深階層で自動取得不可(401)。**CLAUDE.md絶対ルール順守で推測額(戸建てコラボ相場≈¥5,720)は計上しない**。
- **確定の最短手段(やますけ1分)**: My docomo → ご利用料金 → 6月確定分の内訳で「ドコモ光」金額を確認 or eビリング(請求書PDF)を有効化。判明したら external endpoint で固定額自動計上(sourceId=docomo-hikari:{ym})に載せる。それまで the Terrace 通信費は空(現状6月Wi-Fi¥1,200手入力のみ=これも要確認)。

## DONE(2026-07-14 小町水道のMF月割化+Chromeウィンドウ増殖の根治)
- **小町水道をMF月割方式へ切替(やますけ決定)**: 広島市水道はゆうちょ高陽(川原石ラベル)から隔月引落。**引落額を「引落月+前月」に÷2月割**(端数は引落月側)。6/29引落¥3,267 → 5月¥1,633+6月¥1,634 に切替済(旧PDF月割¥1,069×2を除去)。**小町5月利益¥88,785(44%) / 6月利益¥26,658(17.4%)**、報告書再生成済。mf-komachi-utilities ルーチンに水道を追加(次回8月末引落=7月分から自動)。skipFolders に「水道」追加=**小町の光熱PDFで残るのはネットのみ**。
- **出典・内訳のラベル改善**: creditCardIndex 全部を「クレカ電気」表示していたのを、mf:=クレカ明細(MF) / mfbank:=銀行引落(MF) / rakuten:=楽天でんきAPI / それ以外=クレカ明細PDF に分離、費目名も catId から表示、リンクも各ソースへ(デプロイ済)。
- **★Chromeウィンドウ増殖の根治(やますけ指摘)**: 原因=スクリプトの自己修復が「CDP接続失敗のたびに chrome.exe を起動」→ **既存インスタンス生存中の起動は Chrome シングルトン機構で「新ウィンドウを開く」だけ**=開発中のリトライで増殖。対策=3スクリプト共通で **HTTP(/json/version)死亡時のみ起動**、HTTP生存+CDP無応答(stale)は kill→起動。CDP接続タイムアウトも6s→15sに拡大。**PC常駐スクリプトの鉄則(3): 生存中のchrome.exe起動禁止**。
- 楽天でんきマイページのセッションは debug Chrome の再起動(kill含む)を跨いで保持されることを実測確認(cookieベース)。

## DONE(2026-07-14 ルート競合の棚卸しと恒久対策+過去分バックフィル完了確認)
やますけ依頼「最新方式で過去分を埋める+PDF/MF/APIルートの競合有無」→ 全数点検の結論:
**過去分の埋め残し = ゼロ**(本日までの作業で完了): Terrace電気=全月済(楽天でんきAPI 5ヶ月+PDF+MF) / 小町の開業後(05-06)ガス・電気・水道・固定電話=全て計上済(6月ガス¥5,270は本日補完) / Booking売上・手数料・キャンセル料=銀行突合で全月確定 / 12月欠落予約=財務明細で補完済。
- **固定電話5月分の「欠落」は欠落でない**: MF精査(第三+フーガ 3-7月)で NTTファイナンス引落は 4/15¥6,188 と 6/26¥6,729 のみ=**実質隔月請求**。請求発生月に計上する現行のままが正。
**ルート競合(二重計上リスク)の棚卸しと対策**:
1. ✅対策済(本日実装): **Terrace電気 SAISON PDF vs MFルーチン** — importCreditCardElectric に「同一明細対象月(ym)に同額エントリが既存ならスキップ」の対称dedupを追加(経路違いの同一請求を弾く。デプロイ済)。加えて **Terrace に driveUtilitiesSkipFolders=["電気"] を設定**(54電気フォルダのPDFも今後取込対象外=3ルート競合を根絶)。
2. ✅対策済(昨日): **小町のガス/電気/固定電話 PDF vs MFルーチン** — driveUtilitiesSkipFolders=["ガス","電気","固定電話"]+ルーチンはSTART_YM=2026-07以降のみ。
3. ✅設計上競合なし: 入金監視(Booking/Airbnb)=読み取り専用で計上しない / 清掃費=source識別(invoice/drive_pdf/manual)で分離 / 宿泊税=申告書PDF優先→無ければ計算のフォールバック構造。
4. ⚠️運用注意(自動化しない判断): **固定電話がコンビニ払い(フーガ)になった月**は、ルーチン=通知のみ(フーガは個人口座で他契約混在リスク)・PDF=skip対象 → **手動計上が必要**(6月¥6,729はPDF経由で計上済だったのでOK。今後フーガ⚠️通知が来たら手動で)。
**新発見(将来のMF化候補)**: 小町の水道(広島市水道)は**ゆうちょ高陽(川原石ラベル)口座から隔月引落**(3/2¥3,201 / 4/28¥3,206 / 6/29¥3,267)でMFに写っている。MF化する場合は 54水道 の skip 追加とセット(現状はPDF月割ルートが機能しているため保留)。
**未解決の小課題**: Terrace ドコモ光(5/19開通)の月額がどこにも明示計上されていない(6月Wi-Fi¥1,200手入力のみ)。アメックス個人カードの「ドコモご利用料金」¥7,213(4/30)は個人携帯と合算の可能性 → ★やますけに「Terraceドコモ光の月額と支払い元」を確認したい。

## DONE(2026-07-14 MF化4点セット実装・稼働開始 — GO実行完了)
やますけGOを受けて実装・E2E実証・稼働開始。**朝の自動ラインナップは4本体制**(07:20経理ダイジェスト / 07:35 Terrace電気 / 07:37 小町光熱通信 / 07:40 OTA入金監視)。
- **①Airbnb入金監視(mf-booking-monitor.mjs に統合)**: ペイオニア入金を第三(the Terrace)+ハープ(宿小町)で検知→ API `verify-airbnb-payout` が予約CSVの「収入」と照合(単独/2件合算、CI窓=入金日-21日〜+2日)。**一致=無音/不一致のみ🚨/CSV未着=保留(state未登録で翌日再試行)**。E2E: 6月入金12件全一致(謎だった6/8¥22,000もTerrace予約と一致)、7月CI分は正しく保留。口座⇔物件が逆の期間(小町5月前半=第三)もフォールバック照合で対応。
- **②宿小町ガス/③電気/④固定電話(mf-komachi-utilities.mjs 新規、毎日07:37)**: ガス=ハープのニシモトヤ引落(使用月=引落-1) / 電気=楽天でんきマイページAPI(契約8070379292、公式月別料金) / 固定電話=第三のNTTファイナンス引落。**使用月2026-07以降のみ**(過去月PDF計上済と重複させない)。フーガ側でNTTファイナンス支払を見つけたら計上せず⚠️通知(支払方法の揺れ検知)。
- **API新設**: `POST /pnl/:pid/:ym/import-external-utility`(汎用の外部ソース計上口。冪等キー=sourceId、費目名指定、overridden保護) / `POST /pnl/:pid/verify-airbnb-payout`。verify系2本に missingCsvMonths を追加(CSV未着と不一致を区別)。import-utilities に `driveUtilitiesSkipFolders`(物件設定)を追加し、**宿小町=["ガス","電気","固定電話"] 設定済**=これらのPDFフォルダは取込対象外(二重計上防止)。→ **楽天でんき領収書の手動DL・ガス/電話PDFのスキャンは今後不要**(水道・ネットはPDF継続)。
- **バックフィル**: 小町6月ガス¥5,270(7/6ハープ引落、PDF未計上だった)を新エンドポイント実証を兼ねて計上 → **小町6月 経費¥88,271 / 利益¥27,223(17.8%)**、報告書再生成済。Terrace 5月Airbnb CSVが新OTAcsvフォルダに無かった件も旧宿泊税フォルダから発掘・配備(6/1入金2件の照合が解消)。
- **運用開始状態**: state初期化済(過去入金は照合済としてseed)。次の実戦=7月CI分Airbnb入金の照合(8/2のCSV取得後に自動解消)、8月頭のBooking入金、小町7月分ガス(8月上旬引落)/電気(8月初確定)/固定電話(7/15引落)。
