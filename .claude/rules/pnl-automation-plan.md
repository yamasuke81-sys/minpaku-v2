...
## NEXT（順序・ここから再開）
1. **要ユーザー判断**: ①システム利用料(Anthropic)の物件配賦方針 ②the Terraceの固定費(所有ならローン/固定資産税/減価償却をどうpnl計上するか) ③宿小町の家賃68,000で確定か。
2. **各月の実運用**: 帳票モーダルで「領収書/光熱費/清掃費/宿泊税を取込」→報告書/精算書PDF生成。宿小町=精算書、the Terrace=報告書のみ(self)。
3. **完全自動バッチ**: 毎月 OTA CSV取込→各種経費取込→帳票下書き生成 を自動化(最終ゴール)。
