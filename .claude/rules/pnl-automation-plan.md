...
## ★★手数料方針（2026-08-18 ユーザー再確定：利益ベース→**売上ベースへ差し戻し**）— こちらが現行
**現行の算定基礎 = 月間売上高（入金額A − 宿泊税預りB）× 料率**。下の「2026-07-14 利益ベース」の節は**もう有効ではない**（経緯として残す）。
- 正となる契約書 = **2026-05-01付「宿泊施設運営代行契約書」＋別紙2（当初版・月間売上高×50%）**。2026-07-14付の「利益ベース改定版」覚書と、2026-08-03付の簡易版契約（純利益30%）は**いずれも不採用**（8/3版はDriveのゴミ箱へ）
- 契約と本番実装が2026-07-14〜08-18の約1ヶ月ズレていた（紙=売上ベース／アプリ=利益ベース）。**契約側に合わせてアプリを戻した**
- 実装（2026-08-18）: `settlement.js` の buildContext_ は `feeBase` を渡さない（computeSettlement が売上ベースで計算・basis="revenue"）／`pnl.js` の `/pnl/summary` も `computeDepositAmount` 経由の売上ベースへ／精算書PDF・報告書PDF・フロントの精算プレビューの内訳と文言も売上ベースに戻した
- **立替金を新設**: 費目マスタの `advancedByOwner=true`（既定で「固定電話」）は、**甲が支払い・乙が負担**する費目。契約名義を乙へ変更できないNTT回線が該当（別紙3 第2項）。精算書で `手数料(税込) − 立替金 = ご請求金額` として区分表示する。純粋関数は `computeOwnerAdvances`（ota-csv-logic.js）
- **費用負担区分を動かしても手数料額は変わらない**（売上ベースなので）。過去の精算書を作り直す必要はない
- 別紙3 差し替え版（NTT電話料を甲→乙）: Drive `M001_エクセリア小町/008_民泊運用`

## ~~★手数料方針（2026-07-14 ユーザー確定：売上ベース→利益ベースへ全面変更）~~ ← 2026-08-18に差し戻し済（履歴）
- **代行手数料は「利益ベース」で算定する**（旧＝売上ベース `売上高(入金額A−宿泊税B)×料率` は廃止）。
  - 算定基礎 = **運営利益** = 売上 − OTA手数料 − 清掃費 − 諸経費（`computePnl().profit`）。**宿泊税は預り金なので基礎に含めない**。
  - 手数料(税抜) = 運営利益 × 料率（**運営利益が0以下なら手数料0**でフロア）／ 税込 = ×1.1。
  - **月別収支の「利益」= 運営利益 − 手数料(税込) = オーナー最終手取り**（手数料控除後を表示）。運営利益は `operatingProfit` で別途保持。
  - **料率(%)は据え置き**（宿小町50%等）。基礎だけ売上→利益に変更（∴手数料額は大幅減：宿小町5月 税込110,534→48,832）。
  - **全代行物件に適用**（宿小町/若草/宇品）。self(the Terrace等)は手数料0で不変。
- 実装（本番反映済 v0714e / commit 47b9314）: `computeSettlement` に `feeBase` 経路追加（未指定なら旧・売上ベースで後方互換）／`/pnl/summary` は運営利益を feeBase に、profit を netに上書き／`settlement.js` の精算書PDFは「運営利益ベース」内訳、報告書PDFは代行物件に「代行手数料(税込)/オーナー最終利益」行を追記／フロント pnl.js のテーブル見出し「利益(手数料控除後)」・帳票モーダル精算プレビュー・`_recalcDocPreview` を利益ベース化（宿泊税入力は"記録用"に降格＝手数料に非影響）。テスト355緑。
- 既存の下書きPDF（pnlBatchRuns）は旧・売上ベースのまま。**確定/送付前に帳票を再生成すれば新式（利益ベース）になる**（宿小町5/6月は 2026-07-14 に再生成済）。月次バッチ pnlMonthlyImport も同ロジックを使うため以降は自動で利益ベース。
- ★波及要確認: **料率覚書（契約別紙）は旧・売上ベース記載のはず**。利益ベースへ整合させる文面更新が必要（特に第三者=宇品/富永洋 分。ただし宇品は開業前で実請求なし）。
...
- **eビリング自動取得の完全手順を確立**: `https://payment2.smt.docomo.ne.jp/spguide/ebilling/gkfap001.srv?bis=lpb` を直URLで開く(ログイン維持)→月タブ(4/5/6/7月)クリック→**select[name=root_GKFAGS001_DENWABANGOPULLDOWN] で F5392930898(value=3)=光回線を選択**→「表示」→◇合計を読む。3回線=090-7500-9595(ahamo)/070-8488-7966/F5392930898(光)。
- **5月分(開通月)=¥6,988 計上**: 基本使用料1,006(5/19開通の日割り)+その他5,432(初期工事費等)+消費税550。**5月 the Terrace 通信費 ¥0→¥6,988 / 利益¥438,159(65.3%)**。6月=¥6,636 と合わせ両月計上・報告書再生成済。
- **自動化の推奨方式(次NEXT)**: ドコモ光は**ほぼ定額**(6月以降 工事分割終了(2028年春頃)まで¥6,636、以降¥5,720)。eビリングはブラウザ不安定リスクがあるので、**固定費として月次計上**(properties.pnlSettings に月額固定費、pnlMonthlyImport で適用、sourceId=docomo-hikari:{ym}、overridden保護)がブラウザ不要で最堅牢。実額ズレ時だけeビリング手順で検算。5月(6,988)・6月(6,636)は計上済=固定費適用は7月ご利用分から。
...
## DONE(2026-08-10 ★Airbnb月跨ぎ二重計上の根治+2026-07是正 — commit 0a9351f・デプロイ済)
やますけの「Airbnb公式とスプシ売上が違う」を起点に全量調査→構造バグを根治。
- **真因**: AirbnbのCSV期間フィルタは「滞在が対象月にかかる予約」を返すため、月跨ぎ予約が前月・当月の両CSVに載り、全行合算の取込で二重計上。6月是正済みのJessie Lee件と同一構造の再発
- **修正**: `sumAirbnbCsv`/`extractAirbnbReservations`/`sumBookingCsv`/`extractBookingReservations` に `targetYearMonth`(CI月)フィルタ追加、import-ota-csv と宿泊税フォールバックに配線。除外行は `outOfMonthExcludedCount/Total` として revenue に可視化。テスト6件追加(全447緑)。functions:api+pnlMonthlyImport デプロイ済
- **2026-07 是正(再取込)**: テラス ¥809,492→**¥656,232**(8件14泊、Zhang HMKJSHXTQX ¥153,260除外) / 宿小町 ¥200,776→**¥162,920**(6件14泊、연우 신 HMP4SQXJED ¥37,856除外)。過去月(2025-09〜2026-06)は全物件クリーン(全量スキャン済)。報告書PDF再生成済(テラスreport/小町report+settlement)
- **テラス2026-06 宿泊税 ¥9,000 を申告書PDFから取込**(taxWithholding 0→9,000。PDFは7/29からDriveにあったが未取込だった)
- **テラス2026-04 booking(¥46,800キャンセル料・手動値)に protect(revenue.booking) を設定**(最新CSVは0行のため再取込で消える地雷だった)
- **¥19,773(7/6着金)の正体=宿小町 HMRAW52KSN Tayla Basso の正常入金**。帳簿は正しく1回計上・実害なし。mf-booking-monitor state の「テラス/キャンセル料」ラベルはリプレイ時のCSV未着→クロス物件フォールバック誤解釈(要修正: 誤ラベル訂正+CSV未着時は保留に徹する)
- **宿泊税の二重申告なし**(やどぜいは宿泊日ベース配賦+予約単位の重複排除。月計表の日別内訳で実証)
- **★未解決の疑義: 宿泊税テーブル**。実申告書様式は「1人1泊6,000円以上=一律200円」でテラス06(¥9,000)/07(¥15,000)と完全一致するが、宿小町2026-05申告(¥800)はコードの旧テーブル(1万未満非課税/1-2万=200/2万+=500)と一致 → **2026-06頃に税率改定の可能性**。条例・様式を確認するまで `hiroshimaTaxPerPersonPerNight` は変更しない(過去月の計算値も要再検証)

## DONE(2026-08-01 若草の楽天でんきをCONTRACTS登録+取りこぼし対策で複数月バックフィル化)
やますけ報告「若草は8/14〜楽天に切り替わる(お客様番号8089375892・申込2026-07-14)」を受けて `scripts/rakuten-denki-monthly.mjs` を改修。
- **契約ごとに `startYm`(このルーチンが担当する最初の使用月)を新設**。宿小町=`2026-07`(2026-06以前はPDF/MF経由で計上済=二重計上防止)、**若草=`null`(開業前は取得も通知もしない)**。→ **若草の民泊開業月が決まったら startYm を "YYYY-MM" にする(あわせて物件 `pnlBatchEnabled` も要確認)**。開通(8/14)〜開業までの電気は開業準備費で pnl 対象外という既存の会計方針どおり。
- **★取りこぼしバグを解消: 対象月を「前月」単月 → 「startYm以降の未取得月すべて」(遡り上限12ヶ月)へ**。検針日は契約ごとに違う(宿小町=11〜14日 / 呉市清水=月末 / 音戸・西川原石=月初)ため、**月末検針の契約では翌月1〜7日の窓に確定が間に合わず、単月方式だとその月を永久に取り逃す**構造だった(tgtは常に前月のため翌月には対象から外れる)。未取得月は state に残り翌月の窓で自動的に拾い直す。使用量APIは年単位で全月返るので契約×年でキャッシュし、複数月でもリクエストは増やさない。
- 検針待ち⏳通知は**窓の最終日(7日)か手動起動時のみ**(旧: 未確定の間は毎日NOTIFY=月末検針の契約が増えるとノイズになる)。`--check --force` で開業前契約の実在・検針状況も診断可能に(status/同年実績件数を表示)。
