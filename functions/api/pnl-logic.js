...
/**
 * 広島県宿泊税(呉市・広島市共通、令和8年4月1日施行)の1人1泊あたり税額。
 * 単一閾値の一律税率(段階なし):
 *   - 1人1泊 6,000円未満 = 非課税(0円)
 *   - 1人1泊 6,000円以上 = 200円(一律。上位区分は存在しない)
 * 課税基準は「宿泊料金(税抜・素泊まり相当)」だがAirbnb/Bookingは税込表示のケースがあり、閾値をまたぐ稀例外は
 *  導入運用でユーザーが確認する。
 *
 * 根拠(2026-08-10 検証・確定):
 *   - 広島県公式 https://www.pref.hiroshima.lg.jp/site/zei/hiroshima-syukuhakuzei01.html
 *     「6,000円未満は課税なし、6,000円以上は1人1泊につき200円」(単一段階、施行日2026-04-01)
 *   - 実際の納入申告書(様式第3号)の書式自体が「課税対象(１人１泊６千円以上)…200円」の1区分のみで、
 *     上位区分(500円等)の記入欄が存在しない(the Terrace・宿小町 双方の実物PDFで確認)
 *   - the Terrace(呉市) 2026-06(45人泊×200円=9,000円)・2026-07(75人泊×200円=15,000円)を
 *     実予約データ(Airbnb+Booking CSV)から1人泊単位で再計算し、実際の申告書額と完全一致を確認
 *   - 旧実装(10,000円/20,000円の3段階)は上記のいずれとも整合せず、根拠不明な誤りだった
 * (旧・宿小町 2026-05 申告額800円は、正しい計算(2,400円)と食い違っており、申告時点の計算誤りと推定される
 *  ※本テーブルの修正では追随しない。実申告の是非は別途確認が必要)
 */
