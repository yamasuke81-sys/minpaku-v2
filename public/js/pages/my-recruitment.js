...
const MyRecruitmentPage = {
  ...
  renderCalendar() {
    ...
    if (edgePrev && edgeNext) {
      ...
      // 重複バインド防止 (renderCalendar は何度も呼ばれる)
      if (!container._edgeListenerBound) {
        container._edgeListenerBound = true;
        container.addEventListener("scroll", refresh, { passive: true });
        window.addEventListener("resize", refresh, { passive: true });
      }
      // 初回 + 各 render 後 + 自動スクロール (today ジャンプ) 完了後に再評価
      // 自動スクロールはプログラムスクロールで scroll event が安定発火しない
      // 端末があるため、明示的にここで呼ぶ
