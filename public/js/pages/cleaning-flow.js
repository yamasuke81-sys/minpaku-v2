...
const CleaningFlowPage = {
  ...
  _notifDefaults: {
    ...
    // 直前点検完了 (cleaning_done とは独立した通知先設定を可能にする)
    pre_inspection_done:  { defaultMsg: "✨ 直前点検完了\n\n{date} {property}\n{staff}さんが{time}に直前点検を完了しました。\n\n📝 メモ:\n{memos}\n\n📷 写真 ({photoCount}枚):\n{photos}\n\n詳細: {url}", defaultTiming: "immediate", varGroup: "cleaning" },
  ...
  STEPS: [
    // ---- Phase 1: 募集 ----
    {
      ...
      linkLabel: "通知設定",
      // 直前点検も同じ recruit_start チャンネルから募集される
      // 期間/必要人数/開始時間は物件管理画面の「直前点検設定」で設定
      extraLinks: [
        { hash: "#/properties", label: "直前点検設定 (物件管理)" },
      ],
      hint: "通常清掃と直前点検の両方を同じチャンネルから募集します。直前点検の有効化・期間・必要人数・開始時間は物件管理画面で設定してください。",
    },
    {
  ...
  async render(container) {
    container.innerHTML = `
      ...
      <div id="cfPropertySelector" class="rf-property-bar mb-3"></div>
      <!-- メインスイムレーン -->
  ...
  _renderCard(step, property, enabled, memo) {
    ...
    const linkBtn = step.linkHash
      ...
      : "";
    // 追加リンク (extraLinks) — 例: recruit_start カードから直前点検設定 (物件管理) へ
    const extraLinksBtns = Array.isArray(step.extraLinks)
      ? step.extraLinks.map(l =>
          `<a href="${this._esc(l.hash)}" class="btn btn-outline-secondary btn-sm py-0 px-2 ms-1" style="font-size:0.72rem;">${this._esc(l.label || "設定")} <i class="bi bi-arrow-right"></i></a>`
        ).join("")
      : "";
  ...
  _attachEvents(wrap, property) {
    ...
    // NotifyChannelEditor のバインドは _renderSwimLane 側で再描画毎に実施
  },
