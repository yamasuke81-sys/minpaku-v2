...
const SettingsPage = {
    ...
    container.innerHTML = `
      ...
      <div class="card mb-4">
        <div class="card-header bg-primary text-white">
          <h5 class="mb-0"><i class="bi bi-calendar2-event"></i> iCal同期設定</h5>
        </div>
        <div class="card-body">
          <p class="text-muted small">
            Airbnb・Booking.com等のiCalフィードURLを登録すると、30分おきに予約データを自動同期します。<br>
            <strong>Beds24導入後はBeds24 API同期に切り替え、iCal同期は無効化できます。</strong>
          </p>

          <div id="icalSyncList" class="mb-3">
            <div class="text-center py-2 text-muted"><div class="spinner-border spinner-border-sm"></div> 読み込み中...</div>
          </div>

          <div class="card border-dashed mb-3" id="icalAddForm">
            <div class="card-body">
          ...
          <button class="btn btn-outline-primary btn-sm" id="btnSyncIcalNow">
            <i class="bi bi-arrow-repeat"></i> 今すぐ手動同期
          </button>
  ...
  bindEvents() {
    ...
    // iCal同期設定
    document.getElementById("newIcalUrl").addEventListener("input", (e) => {
      ...
      document.getElementById("newIcalPlatform").value = platform;
    });
    document.getElementById("btnAddIcalUrl").addEventListener("click", () => this.addIcalUrl());
...
idxはそのまま返してください。optionsとplaceholderは入力にある場合のみ返してください。`;
  ...
  async loadIcalSettings() {
    const listEl = document.getElementById("icalSyncList");
    try {
      ...
      // 有効/無効トグル
      listEl.querySelectorAll(".btnToggleIcal").forEach(btn => {
  ...
  async addIcalUrl() {
    ...
    const platform = document.getElementById("newIcalPlatform").value.trim();
    const propertyId = document.getElementById("newIcalPropertyId").value.trim();

    if (!url) {
      showToast("エラー", "iCal URLを入力してください", "error");
      return;
    }
    ...
    try {
      ...
      document.getElementById("newIcalPlatform").value = "";
      document.getElementById("newIcalPropertyId").value = "";
  ...
  async syncIcalNow() {
    ...
    alertEl.innerHTML = '<div class="spinner-border spinner-border-sm me-2"></div>手動同期を実行中...（Cloud Function経由）';

    try {
      // Cloud Functionを直接呼び出す（callable function経由）
      // スケジュール関数は直接呼べないため、syncSettingsの状態確認のみ表示
      const snap = await db.collection("syncSettings").get();
      if (snap.empty) {
      ...
      snap.forEach(doc => {
        const d = doc.data();
        if (d.active !== false) {
          settings.push({ platform: d.platform, lastSync: d.lastSync, result: d.lastSyncResult });
        }
      });
      ...
      let html = '<strong>登録済みiCal同期（30分おきに自動実行）:</strong><ul class="mb-0 mt-1">';
      settings.forEach(s => {
        const lastSync = s.lastSync ? new Date(s.lastSync.seconds * 1000).toLocaleString("ja-JP") : "未同期";
        html += `<li>${s.platform}: 最終同期 ${lastSync} ${s.result ? `(${s.result})` : ""}</li>`;
      });
      html += '</ul><small class="text-muted">※ 手動トリガーはFirebaseコンソールまたはCLIから実行できます。次回の自動同期は最大30分後です。</small>';
      alertEl.className = "alert alert-info py-1 small";
