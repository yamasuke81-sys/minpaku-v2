...
(function(global) {
  ...
  const SYSTEM_VARIABLES = {
    recruit: [
      ...
      { name: "url",      label: "回答ページURL", sample: "https://app.setouchi-stay.com/#/my-recruitment/{recruitmentId}", source: "自動生成 (タップで該当募集の詳細モーダルを直接オープン)" },
    ...
    booking: [
      ...
      { name: "url",      label: "名簿ページURL",   sample: "https://app.setouchi-stay.com/#/guests", source: "自動生成" },
      { name: "editUrl",  label: "名簿修正用URL (ゲスト用)", sample: "https://app.setouchi-stay.com/guest-form.html?edit=...&propertyId=...", source: "guestRegistrations.editToken (30日間有効)" },
    ],
    staff: [
      ...
      { name: "url",      label: "マイページURL", sample: "https://app.setouchi-stay.com/#/my-dashboard", source: "自動生成" },
    ...
    invoice: [
      ...
      { name: "url",      label: "確認/作成ページURL", sample: "https://app.setouchi-stay.com/#/invoices/{invoiceId}", source: "請求書要請: /#/my-invoice-create、提出通知/確定通知: /#/invoices/{invoiceId} (タップで該当請求書の詳細モーダルを直接オープン)" },
    ],
    cleaning: [
      ...
      { name: "url",        label: "チェックリストURL", sample: "https://app.setouchi-stay.com/#/my-checklist/xxx", source: "自動生成 (該当シフトのチェックリストページ)" },
      ...
      { name: "photos",     label: "写真URL (最大5枚)", sample: "https://...\nhttps://...", source: "checklist.beforePhotos[] / afterPhotos[] / notes[].photoUrls[]" },
      { name: "photoCount", label: "写真枚数",         sample: "8",            source: "上記写真URLの合計枚数" },
    ],
    laundry: [
    ...
    morning_briefing: [
      ...
      { name: "url",            label: "管理画面URL",       sample: "https://app.setouchi-stay.com/", source: "固定" },
    ],
    parking: [
  ...
  const NOTIFICATIONS = [
    ...
    { key: "booking_change_email", label: "Airbnb変更メール検知", desc: "Airbnbから予約変更承認/変更希望メールを受信したときにWebアプリ管理者へ通知（本文に名簿との人数食い違いも並記）。手動で人数修正するトリガーに使う", icon: "bi-envelope-arrow-up", group: "booking", varGroup: "booking", defaultTiming: "immediate",
      defaultEnabled: true, defaultOwnerLine: true, defaultGroupLine: false, defaultStaffLine: false, defaultEmail: true,
      defaultMsg: "🔄 Airbnb予約変更検知\n\n物件: {property}\nゲスト: {guest}\n予約コード: {code}\n現在の予約: {checkin} 〜 {date}\n予約の登録人数: {booking_count}人\n名簿の申告人数: 合計{roster_total}人 (大人{roster_adults}/子ども{roster_children}/乳幼児{roster_infants}) {roster_count}件\n\n→ Airbnbで内容を確認し、必要なら人数を手動で更新してください。" },
    ...
    { key: "passport_upload_failed", label: "パスポート写真 アップロード失敗", desc: "ゲストフォームでパスポート写真のアップロードが3回連続で失敗した時、Webアプリ管理者等へ通知 (フォローアップ用)", icon: "bi-image-alt", group: "booking", varGroup: "booking", defaultTiming: "immediate",
      defaultEnabled: true, defaultOwnerLine: true, defaultGroupLine: false, defaultStaffLine: false, defaultEmail: true,
      defaultMsg: "📷 パスポート写真 アップロード失敗 (3回連続)\n\n物件: {property}\nゲスト: {guest} ({email})\nエラー: {error}\n\nゲストはご予約サイト経由でホストに連絡するよう案内されています。フォローしてください。" },
    ...
    { key: "invoice_confirmed", label: "請求書承認通知", desc: "Webアプリ管理者が請求書を承認した時にWebアプリ管理者/物件オーナーへ通知", icon: "bi-check2-circle", group: "invoice", varGroup: "invoice", defaultTiming: "immediate",
      defaultEnabled: true, defaultOwnerLine: true, defaultGroupLine: false, defaultStaffLine: false, defaultEmail: true,
